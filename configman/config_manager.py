...
class ConfigurationManager(object):
    ...
    #--------------------------------------------------------------------------
    def _create_symbolic_links(self, keys, known_keys):
        """this method steps through the option definitions looking for
        alt paths.  On finding one, it creates the symbolic_links within the
        option definitions and populates it with copied options."""
        set_of_symbolic_links = set()  # a set of known symbolic_links
        for key in keys:
            if key not in known_keys:  # skip all keys previously seen
                ...
                if (an_option.symbolic_link
                    and an_option.symbolic_link not in set_of_symbolic_links
                    and an_option.symbolic_link not in known_keys):
                    alt_option = an_option.copy()
                    an_option.comment_out = True
                    alt_option.symbolic_link = None
                    alt_option.name = '.'.join(
                        (an_option.symbolic_link, alt_option.name)
                    )
                    set_of_symbolic_links.add(alt_option.name)
                    self.option_definitions.add_option(alt_option)
        for a_symbolic_link in set_of_symbolic_links:
            for x in range(a_symbolic_link.count('.')):
                namespace_path = a_symbolic_link.rsplit('.', x + 1)[0]
                print a_symbolic_link, namespace_path, x + 1
                self.option_definitions[namespace_path].tag_as_symbolic_link()
        return set_of_symbolic_links
    ...
    def _overlay_expand(self):
        ...
        while new_keys_discovered:  # loop until nothing more is done
            ...
            set_of_symbolic_links = self._create_symbolic_links(keys, known_keys)
            all_keys = list(set_of_symbolic_links) + keys
            ...
            for key in all_keys:
                if key not in known_keys:  # skip all keys previously seen
                    ...
                    if self.option_definitions[key].symbolic_link:
                        symbolic_link = self.option_definitions[key].symbolic_link
                        top_key = key.split('.')[-1]
                        self.option_definitions[key].default = \
                            self.option_definitions[symbolic_link][top_key].default
