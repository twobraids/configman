...
class ValueSource(object):
    ...
    @staticmethod
    def _namespace_symbolic_link_sort(key_value_tuple):
        key, value = key_value_tuple
        if value._symbolic_link:
            return 'aaaaaa' + key
        else:
            return key
    ...
    def _write_ini(source_dict, namespace_name=None, level=0, indent_size=4,
        ...
        for an_option in options:
            ...
            if an_option.comment_out:
                ...
                if an_option.symbolic_link:
                    print >>output_stream, (
                        "%s#   it is symbolically linked to another "
                    ...
                    if namespace_name:
                        print >>output_stream, (
                            ...
                            (indent_spacer, namespace_name, an_option.name,
                             an_option.symbolic_link, an_option.name)
                        )
                    else:
                        print >>output_stream, (
                            "%s#   %s -> %s.%s"  %
        ...
        namespaces.sort(key=ValueSource._namespace_symbolic_link_sort)
        for key, namespace in namespaces:
            ...
            if namespace._symbolic_link:
                print >>output_stream, (
                    "%s# this section contains Options that are common in"
                    % next_level_spacer
                )
                print >>output_stream, (
                    "%s# other sections of this ini file.  If they are common"
                    % next_level_spacer
                )
                print >>output_stream, (
